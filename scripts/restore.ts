...
import { db as sql, organizations, chats, messages, knowledge, activities, products, staff, stagingProducts, systemLogs, dailySnapshots, transactions, cartItems } from './packages/database/dist/index.js';
...
async function restore() {
  ...
  for (const doc of chatsSnapshot.docs) {
    const data = doc.data();
    if (!targetOrgs.includes(data.organizationId)) continue;

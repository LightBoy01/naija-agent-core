                  ...
                  <div className="p-4">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-[10px] text-zinc-400 font-mono">
                        {item.timestamp ? formatTimestamp(item.timestamp).toLocaleDateString() : '--'}
                      </span>
                    </div>
                    ...
                    <p className="text-xs text-zinc-500 line-clamp-2">
                      {(item.metadata?.caption as string) || 'No caption provided.'}
                    </p>

                    {/* Operational Risk Mitigation: Manual Archive */}
                    {(item.metadata?.mediaId as string) && (
                      <ArchiveButton 
                        orgId={currentOrgId} 
                        chatId={item.chatId!} 
                        messageId={item.id} 
                        mediaId={item.metadata!.mediaId as string} 
                        type={item.type} 

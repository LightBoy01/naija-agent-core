...
export default function NewProductForm({ orgId, symbol = '₦', entityDef }: NewProductFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await createProductAction(orgId, formData);
    ...
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-zinc-100 p-8 animate-in fade-in zoom-in duration-200">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-black text-zinc-900 uppercase tracking-tighter">New {entityDef.name}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {entityDef.fields.filter(f => f.key !== 'image').map(field => (
            <div key={field.key}>
              <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1 px-1">
                {field.label} {field.key === 'price' ? `(${symbol})` : ''} {field.required ? '' : '(Optional)'}
              </label>
              ...
              {field.description && (
                <p className="text-[10px] text-zinc-400 mt-1 px-1">{field.description}</p>
              )}
            </div>
          ))}
          ...
          >
            {isPending ? 'Saving...' : `Create ${entityDef.name}`}
          </button>
        </form>
      </div>
    </div>

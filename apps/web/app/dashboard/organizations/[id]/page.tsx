        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <section className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 bg-zinc-50 rounded-xl border border-zinc-100">
                <p className="text-xs text-zinc-500 mb-2 font-medium">Auto-Matching Engine</p>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-zinc-900">{org.config?.useSmsBridge ? 'SMS Bridge ON' : 'SMS Bridge OFF'}</span>
                  <form action={updateOrgStatus}>
                    <input type="hidden" name="orgId" value={org.id} />
                    <input type="hidden" name="type" value="bridge" />
                    <input type="hidden" name="status" value={org.config?.useSmsBridge ? 'false' : 'true'} />
                    <button type="submit" className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                      org.config?.useSmsBridge 
                        ? 'text-zinc-600 border-zinc-200 hover:bg-white' 
                        : 'text-blue-600 border-blue-200 hover:bg-blue-50'
                    }`}>
                      {org.config?.useSmsBridge ? 'Disable Bridge' : 'Enable Bridge'}
                    </button>
                  </form>
                </div>
              </div>

...
export default async function ChatDetailPage({ params }: PageProps) {
  ...
  const chatInfo = chatDoc.data() as Chat;
            ...
            >
              <div className={`max-w-[80%] rounded-2xl p-4 shadow-sm ${
                ...
                <p className="text-sm whitespace-pre-wrap leading-relaxed">
                  ...
                  {(msg.metadata?.caption as string) && <span className="block mt-2 font-medium italic opacity-80">&quot;{msg.metadata?.caption as string}&quot;</span>}
                </p>

                {/* Timestamp */}
                <div className={`text-[10px] mt-2 font-medium opacity-50 ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
                  {msg.timestamp ? formatTimestamp(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--'}
                </div>
              </div>
            </div>
          ))

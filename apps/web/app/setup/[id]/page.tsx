...
export default function SetupPage() {
  ...
  const handleQRSuccess = (data: { connected: true }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // In Sovereign mode, we just need the QR to be connected
    // The Sidecar already handles the tokens.
    if (!whatsappConnected) {
      toast.error('Please scan the QR code to connect WhatsApp first!');
      return;
    }
    ...
    setIsFinishing(true);
    try {
      // Future step: send this to Fastify to create Organization record
      // For now, bypass Firebase and just show success!
      const res = await fetch('/api/proxy/sidecar/connect', {
         method: 'OPTIONS', // dummy check to simulate finalization
      });
    ...
    <main className="min-h-screen bg-zinc-50 py-12 px-6 font-sans">
      <div className="max-w-md mx-auto bg-white rounded-2xl shadow-xl border border-zinc-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-200 p-4 rounded-lg text-xs text-amber-800 space-y-2">
              <p className="font-bold flex items-center gap-1">
                ...
                SOVEREIGN ENGINE REQUIREMENT
              </p>
              <p>
                You need a <b>New WhatsApp Account</b> (or WhatsApp Business) for your AI Bot.
              </p>
            </div>

            {/* Sovereign QR Scanner replaces Meta Embedded Signup */}
          ...
          <button
            type="submit"
            disabled={isFinishing || !whatsappConnected}

...
CREATE TABLE IF NOT EXISTS "fraud_registry" (
	...
	"evidence_url" text,
	"created_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "fraud_registry_phone_hash_org_id_pk" PRIMARY KEY("phone_hash","org_id")
);
--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "memories_embedding_idx" ON "memories" ("embedding");--> statement-breakpoint
DO $$ BEGIN
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "fraud_registry" ADD CONSTRAINT "fraud_registry_org_id_organizations_id_fk" FOREIGN KEY ("org_id") REFERENCES "organizations"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint

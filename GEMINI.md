...
- **Active AI Stack:** Multi-provider via `@naija-agent/ai` capability router — Zynux: `deepseek-v4-flash`, Aelixxr: `deepseek-v4-pro`, Router: `gemini-3-flash-preview`, Fallback: `gemini-3.1-flash-lite`.
- **Audio Processing:** Verified (Session 13). Correctly transcribes/responds to voice notes.
- **Image Processing:** Verified (Session 14). Gemini Vision analyzes images/receipts.
- **Outbound System:** Live. Sidecar POST endpoints verified in production.
- **Billing:** Energy credits (Kobo-based) with immutable `energy_ledger` audit trail.

## Recent Changes
- **Phase 9.6 AI Capabilities & Sovereign Fixes (June 2026):**
    - **Aelixxr Tool Unchaining:** Removed the hardcoded `allowedNames` filter in `apps/worker-life/src/tools/index.ts` to natively expose `web_search` and `delegate_to_hermes` to the Aelixxr Orchestrator.
    - **Zynux Sovereign Authority:** Fixed Zynux's database `config` (`isMaster: true`) in PostgreSQL so he correctly loads the protected `SYSTEM_TOOLS`.
    - **Unified Container Media Resolution:** Discovered that the Go `whatsapp-sidecar` and Node `worker` share a single filesystem via `sovereign-start.sh`. Rewrote Zynux's `downloadMediaFromSovereign` to natively read `/tmp/` image paths generated by the sidecar, bypassing broken REST calls and fixing image uploads.
    - **Timezone Context Fix:** Injected `Date.now()` into system prompts to prevent the AI from generating reminders with past timestamps.
- **Phase 9.4 AI Ecosystem Evolution (Implemented):**
    - **Capability-Based Dynamic Router:** Active multi-provider routing — selects cheapest capable model per task (reasoning, tool-calling, vision, audio) with automatic failover across providers.
    - **Multi-Client Strategy (Titan-to-Titan):** Integrated **DeepSeek-V4-Pro/Flash** for 1M-context tasks and **Alibaba DashScope (Qwen3)** for specialized workloads.
    - **Gemini Preservation:** Gemini 3 Flash used for capability routing and as fallback; DeepSeek handles primary worker tasks.
...
## Next Steps (Phase 10)
- [ ] **Data Migration Completion:** Migrate the `knowledge` and `products` tables from Firebase to PostgreSQL to eliminate Zynux's split-brain architecture.
- [ ] **RAM Caching:** Build memory loader with `fs.watch` for hot-reloading markdown prompts.
- [ ] **Sector Packs Execution:** Implement dynamic routing of both Native and MCP tools using Sector Packs.
- [ ] **Test Coverage:** Expand test coverage to `@naija-agent/ai`, `@naija-agent/payments`, and `apps/api`.
...
- [x] **Energy Ledger:** Immutable `energy_ledger` table with full audit trail deployed.
